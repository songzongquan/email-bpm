    ...
    def main(self):
        ...
        while True:#五分钟循环一次
            ...
            for attachment in attachmentlist:
                ...
                executescript1 = executescript.split(' ') 
                if ".sh" in executescript:
                ...
                elif ".py" in executescript:
                    status = subprocess.Popen(executescript1,stdout=subprocess.PIPE,cwd=os.getcwd() + "/script/").communicate()
                    status1=str(status[0])
                    status2=status1.split('\\n')
                    excelparse.write("执行结果", status2[1])
