    ...
    def main(self):
        while True:
            ...
            for attachment in attachmentlist:
                '''for循环，输出邮件中的所有附件'''
                print(attachment)
                attachmentname = e.getFileNameOfAttachement(id)     #调用获得附件名字的方法，获得所有附件的名称
                print(attachmentname)
                abspath = os.path.abspath(os.path.dirname(os.path.dirname(__file__)))     #当前文件的绝对路径的上一级
                downloadpath = os.path.join(abspath+ "/host_auth_Attachments")       #确定附件下载的地址
                downloadatta = e.downloadAttachement(attachmentname,downloadpath)    #调用下载附件的方法，下载附件
                print(downloadatta)

                attachmentpath = os.path.join(downloadpath + attachmentname)        #确定精确到附件名的路径
                p = ExcelReadWriter(attachmentpath)       #实例化ExcelReadWriter类
                if "form_堡垒机授权申请" in attachmentname:
